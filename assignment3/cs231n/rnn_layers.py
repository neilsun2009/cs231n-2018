...
def rnn_step_backward(dnext_h, cache):
    ...
    daffine = dnext_h * (1 - next_h * next_h)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        next_h, cache_now = rnn_step_forward(x[i], prev_h, Wx, Wh, b)
        h.append(next_h)
        prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        dx_now, dprev_h, dWx_now, dWh_now, db_now = rnn_step_backward(dnext_h, cache[i])
        dx.append(dx_now)
